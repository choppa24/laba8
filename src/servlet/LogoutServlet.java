...
public class LogoutServlet extends ChatServlet { 
  ...
  private static final long serialVersionUID = 1L; 
     //Другими словами, выполняется проверка,
     // тот ли пользователь пытается выйти из чата, что и входил в него.
  protected void doGet(HttpServletRequest request, HttpServletResponse 
   ...
   if (name!=null) {
     ...
     ChatUser aUser = activeUsers.get(name);
      ...
      response.sendRedirect(response.encodeRedirectURL("/laba80_war_exploded/"));
     ChatUser.setKol(1);
      } else {
          // Пользователь пытается аннулировать чужую сессию –
            // не делать ничего
      response.sendRedirect(response.encodeRedirectURL("/laba80_war_exploded/view.htm"));
    	  } 
      } else {
